...
### [Open the live ReconcileAI demo →](https://reconcileai-production-65ca.up.railway.app/)

The public Railway deployment runs in credential-free **Demo Mode** using only the deterministic synthetic dataset and `MockAIProvider`. It makes no live Razorpay, LLM, or financial calls.
...
**Live demo:** [https://reconcileai-production-65ca.up.railway.app/](https://reconcileai-production-65ca.up.railway.app/)

The public demo is deployed from this GitHub repository on Railway's Free Trial using the root `Dockerfile`. It is built with Node.js 22 and runs as a single Python application instance, with a persistent volume mounted at `/data` and SQLite stored at `/data/reconcileai.db`. No Razorpay or LLM credentials are configured, so the deployed application remains in deterministic, credential-free Demo Mode.
...
The production startup path was first verified locally using the exact container command (`uvicorn app.main:app --host 0.0.0.0 --port $PORT` against the built `frontend/dist`), including the full Playwright judge journey and ₹9.83 safety-case suite. It is now also exercised by the public Railway deployment above. A safe Railway restart confirmed that `/data/reconcileai.db` and its hash-chained audit records persist through container restarts. The richer `RunRegistry` remains intentionally process-lifetime only, so the Runs list is repopulated by starting a new deterministic synthetic run after a restart.
...
- A public Railway demo now exercises the `Dockerfile` deployment path (see [Deployment](#deployment)). It uses one demo instance and persistent SQLite storage, but no automated backup, monitoring, or disaster-recovery infrastructure is claimed.
