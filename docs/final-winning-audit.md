...
| Frontend flow | `frontend/src/pages/*` | Confirmed via the release-hardened Vitest suite (22/22) and real-browser E2E suite (2/2) |
...
| 7 | Technical depth | 9 | 862 backend tests, 16 engineering milestones each with dated architectural decisions and empirically-found bugs (not just passing tests written after the fact) | Depth risk is presentation, not substance — a judge skimming the repo for 2 minutes could miss it entirely without a guide | Already mitigated by `docs/final-validation.md`'s "exact commands to reproduce" section |
...
| 12 | Demo impact | 8 | A synthetic, reproducible ₹9.83 golden case with a full CONTRADICTED→REJECTED/BLOCKED→AUDITED path | Impact depends entirely on the presenter executing the demo script well | See Phase 8/9 below — demo failure planning matters more here than more code |
...
| 16 | Reliability | 9 | 862/862 backend, 22/22 frontend, 2/2 E2E; release hardening now directly covers simultaneous run starts and the exact synthetic ₹9.83 blocked case | No distributed-write or production-scale load test | Low priority — the single-process demo boundary is explicit |
...
Best answer: This is the golden demo case — the synthetic ₹9.83 scenario. AI's fee-explains-difference hypothesis is checked against the configured `FeeRule`; when it does not reconcile, the BLOCK tier returns `REJECTED` via `POLICY-VERIFIER-FAIL-001`, with no action. Code evidence: `backend/tests/policy/test_resolution_service.py`, `docs/final-validation.md` §11. Demo: walk through the contradiction, block, and audit trail live.
...
**Honest flag, not hidden:** 74/300 cases are routed through the deterministic `MockAIProvider` interface, and 75% of the tested mock hypotheses are rejected by independent verification. This measures the verification boundary, not live-model accuracy. The deterministic layer already scores 1.0000 precision/recall on this synthetic dataset, and the Baseline D proxy produces zero direct auto-resolve recommendations; therefore no AI-driven accuracy or automation lift is claimed. No live LLM was evaluated.
...
Verified against `docs/three-minute-demo.md` and the release-hardened Playwright E2E suite (2/2 passing, including the exact synthetic ₹9.83 safety case in a real browser):
...
| 5. Exception | Work Queue `Blocked` fee-mismatch row | Confirmed (E2E suite filters and opens this exact scenario) |
...
| 10. Safe block | Final Decision: BLOCKED / `REJECTED` | Confirmed; no financial action or review request is created |
| 11. Audit/provenance | "Audit & Provenance" heading | Confirmed (E2E asserts this heading) |
| 12. Priority | Work Queue priority badge + Exception Detail priority panel | Confirmed |
...
**Every screen named in the demo script is a screen the E2E suite exercises in a real browser.** The ₹9.83 hero case is synthetic and reproducible from the committed dataset; it is not a real merchant transaction.
