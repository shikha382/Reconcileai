...
export const mockExceptionDetail: ExceptionDetailResponse = {
  ...
  policy: { decision: "HUMAN_REVIEW", policy_id: "POLICY-CONFLICT-001", policy_version: "1.0", risk_level: "CRITICAL", required_approval: true, reasons: ["Conflicting evidence detected."], blocked_reasons: [] },
  decision: "HUMAN_REVIEW",
  review: { review_id: "REV-1", state: "PENDING", dual_control_required: false, created_by: "system" },
...
export const mockExplanation: ExplanationResponse = {
  ...
  policy: { policy_id: "POLICY-CONFLICT-001", policy_version: "1.0", decision: "HUMAN_REVIEW", rules_evaluated: ["POLICY-CONFLICT-001"], rules_passed: [], rules_failed: ["POLICY-CONFLICT-001"], reasons: ["Conflicting evidence detected."], blocked_reasons: [], required_approval: true, risk_level: "CRITICAL" },
  risk: { risk_level: "CRITICAL", risk_score: "199580.08", reasons: ["High financial delta", "Contradicted hypothesis"] },
  resolution: { resolution_type: "REVIEW_REQUIRED", requires_approval: true, review_state: "PENDING", dual_control_required: false, financial_impact: "9.83" },
  ...
  human_readable: "Payment PAY-1 shows a ₹9.83 residual that the AI's fee-adjustment hypothesis does not explain once verified deterministically. Routed to human review.",
  priority: { priority: "P0", priority_score: "199580.08", reason_codes: ["CRITICAL_RISK", "SLA_BREACHED", "CONTRADICTORY_EVIDENCE"], recommended_action: "REVIEW_EVIDENCE" },
};
